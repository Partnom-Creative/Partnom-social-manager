...
const PLATFORM_STYLES: Record<
  string,
  { bg: string; hover: string; icon: React.ReactNode }
> = {
  twitter: {
    bg: "bg-black",
...
export function ConnectAccountButton({
  ...
  function handleConnect() {
    ...
    window.location.href = `/api/oauth/${platformKey}?${params.toString()}`;
  }
  ...
  return (
    <button
      ...
      className={cn(
        "group flex w-full items-center gap-3 rounded-lg px-4 py-3 text-white transition-all",
        style?.bg ?? "bg-slate-800",
        style?.hover ?? "hover:bg-slate-700",
    ...
    >
      ...
      <span className="text-sm font-medium">Connect {platformName}</span>
      <ExternalLink className="ml-auto h-4 w-4 opacity-50 group-hover:opacity-80 transition-opacity" />
    </button>
  );
}
